...
import relog from "/src/assets/images/reglog.svg";

function RegisterPage() {
  const navigate = useNavigate();
  const [createUserWithEmailAndPassword, user, loading, error] = useCreateUserWithEmailAndPassword(auth);

  function registrationFireBaseHook({ email, password, username }) {
    const loadingToastId = toast.loading("Registering...");
    const useris = username;
    ...
    <div className="mt-20 box-border max-sm:mt-10">
      ...
      <div className="flex flex-col items-center">
        <div className="flex flex-col items-center px-6 py-10 max-w-1/2 mb-10 bg-white text-grey rounded-[16px]">
          ...
          <img className="absolute top-[80px] max-sm:top-[30px] z-0" src={relog} alt="" />
